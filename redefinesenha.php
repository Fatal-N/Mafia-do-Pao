...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if ($cod==""){ //validação se o campo cod foi digitado nulo
        header("Location:redefinesenha.php?msg=Cod Inválido");
        exit();
    }
    ...
    if($cont == 0){
        ...
        header("Location:redefinesenha.php?msg=Código errado, solicite outro!");
        exit();
    }
    else{
        $random = rand(100000,999999);
        $tempero = md5(rand() . date('H:i:s')); //novo tempero
        $senha = md5($tempero . $senha); //criptografia senha
